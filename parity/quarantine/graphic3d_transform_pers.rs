...
use crate::graphic3d_trans_mode_flags::TransModeFlags;
...
impl TransformPers {
    ...
    pub fn is_zoom_or_rotate(mode: TransModeFlags) -> bool {
    ...
    pub fn is_trihedron_or_2d(mode: TransModeFlags) -> bool {
        (mode.bits() & (TransModeFlags::TRIHEDRON_PERS.bits() | TransModeFlags::2D.bits())) != 0
    }

    /// Return true if specified mode is orthographic projection transformation persistence.
    pub fn is_ortho_pers(mode: TransModeFlags) -> bool {
        (mode.bits() & TransModeFlags::ORTHO_PERS.bits()) != 0
    }

    /// Return true if specified mode is axial transformation persistence.
    pub fn is_axial(mode: TransModeFlags) -> bool {
    ...
    pub fn new(mode: TransModeFlags) -> Self {
        ...
        let (params_3d, params_2d) = if Self::is_zoom_or_rotate(mode) || Self::is_axial(mode) {
            (
                PersParams3d {
                    pnt_x: 0.0,
                    pnt_y: 0.0,
        ...
        } else if Self::is_trihedron_or_2d(mode) {
            (
                PersParams3d {
                    pnt_x: 0.0,
                    pnt_y: 0.0,
    ...
    pub fn new_with_anchor(mode: TransModeFlags, pnt_x: f64, pnt_y: f64, pnt_z: f64) -> Self {
        if !Self::is_zoom_or_rotate(mode) && !Self::is_axial(mode) {
    ...
    pub fn new_with_corner(
        ...
        if !Self::is_trihedron_or_2d(mode) {
...
mod tests {
    ...
    fn test_is_zoom_or_rotate_flag() {
        assert!(TransformPers::is_zoom_or_rotate(TransModeFlags::ZOOM_PERS));
        assert!(TransformPers::is_zoom_or_rotate(TransModeFlags::ROTATE_PERS));
        assert!(TransformPers::is_zoom_or_rotate(TransModeFlags::ZOOM_ROTATE_PERS));
        assert!(!TransformPers::is_zoom_or_rotate(TransModeFlags::TRIHEDRON_PERS));
    }

    #[test]
    fn test_is_trihedron_or_2d_flag() {
        assert!(TransformPers::is_trihedron_or_2d(TransModeFlags::TRIHEDRON_PERS));
        assert!(TransformPers::is_trihedron_or_2d(TransModeFlags::2D));
        assert!(!TransformPers::is_trihedron_or_2d(TransModeFlags::ZOOM_PERS));
    }

    #[test]
    fn test_is_ortho_pers_flag() {
        assert!(TransformPers::is_ortho_pers(TransModeFlags::ORTHO_PERS));
        assert!(!TransformPers::is_ortho_pers(TransModeFlags::ZOOM_PERS));
    }

    #[test]
    fn test_is_axial_flag() {
        assert!(TransformPers::is_axial(TransModeFlags::AXIAL_SCALE_PERS));
        assert!(TransformPers::is_axial(TransModeFlags::AXIAL_ZOOM_PERS));
        assert!(!TransformPers::is_axial(TransModeFlags::ZOOM_PERS));
    }
