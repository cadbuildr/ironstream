// FILE: transfer_sequence_of_binder.rs
// occt: Transfer_SequenceOfBinder

use std::fmt;

/// Placeholder for Transfer_Binder handle (shared pointer)
#[derive(Clone, Debug)]
pub struct TransferBinderSeqHandle {
    // In real implementation, would contain reference-counted pointer to Transfer_Binder
}

/// Deprecated typedef: NCollection_Sequence<opencascade::handle<Transfer_Binder>>
///
/// A sequence of Transfer_Binder handles.
...
#[derive(Clone, Debug)]
pub struct TransferSequenceOfBinder {
    items: Vec<TransferBinderSeqHandle>,
    lower: i32,
}

impl TransferSequenceOfBinder {
    /// Create an empty sequence.
    pub fn new() -> Self {
        TransferSequenceOfBinder {
            items: Vec::new(),
            lower: 1,
        }
    }
    ...
    pub fn append(&mut self, elem: TransferBinderSeqHandle) {
        self.items.push(elem);
    }

    /// Prepend an element to the sequence.
    pub fn prepend(&mut self, elem: TransferBinderSeqHandle) {
        self.items.insert(0, elem);
    }

    /// Insert element at position (1-based OCCT indexing).
    pub fn insert(&mut self, index: i32, elem: TransferBinderSeqHandle) {
    ...
    pub fn value(&self, index: i32) -> Option<&TransferBinderSeqHandle> {
        if index < self.lower {
            return None;
        }
    ...
    pub fn change_value(&mut self, index: i32) -> Option<&mut TransferBinderSeqHandle> {
        if index < self.lower {
            return None;
        }
    ...
    /// Get iterator.
    pub fn iter(&self) -> impl Iterator<Item = &TransferBinderSeqHandle> {
        self.items.iter()
    }

    /// Get mutable iterator.
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut TransferBinderSeqHandle> {
        self.items.iter_mut()
    }
...
impl Default for TransferSequenceOfBinder {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for TransferSequenceOfBinder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "TransferSequenceOfBinder(size={})", self.size())
    }
}
...
mod tests {
    ...
    fn test_create_empty_sequence() {
        let seq = TransferSequenceOfBinder::new();
        assert!(seq.is_empty());
    ...
    fn test_append_elements() {
        let mut seq = TransferSequenceOfBinder::new();
        let elem = TransferBinderSeqHandle {};
    ...
    fn test_prepend_element() {
        let mut seq = TransferSequenceOfBinder::new();
        seq.append(TransferBinderSeqHandle {});
        seq.append(TransferBinderSeqHandle {});
        seq.prepend(TransferBinderSeqHandle {});
    ...
    fn test_insert_element() {
        ...
        seq.append(TransferBinderSeqHandle {});
        seq.append(TransferBinderSeqHandle {});
        seq.insert(2, TransferBinderSeqHandle {});
        assert_eq!(seq.size(), 3);
    }
