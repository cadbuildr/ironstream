// FILE: t_col_std_h_sequence_of_real.rs
// occt: TColStd_HSequenceOfReal

/// TColStd_HSequenceOfReal is a deprecated alias for a handle (heap-allocated) sequence of real numbers.
/// This is a Rust port implementing OCCT's sequence semantics (1-based indexing).
pub struct TColStdHSequenceOfReal {
    data: Vec<f64>,
}

impl TColStdHSequenceOfReal {
    /// Creates a new empty sequence.
    pub fn new() -> Self {
        TColStdHSequenceOfReal {
            data: Vec::new(),
        }
    }

    /// Appends an element to the sequence.
    pub fn append(&mut self, value: f64) {
        self.data.push(value);
    }

    /// Prepends an element to the sequence.
    pub fn prepend(&mut self, value: f64) {
        self.data.insert(0, value);
    }

    /// Inserts an element at the given 1-based index.
    pub fn insert_before(&mut self, idx: i32, value: f64) {
        if idx < 1 || idx as usize > self.data.len() + 1 {
            panic!("Insert index {} out of range [1, {}]", idx, self.data.len() + 1);
        }
        self.data.insert((idx - 1) as usize, value);
    }

    /// Removes an element at the given 1-based index.
    pub fn remove(&mut self, idx: i32) -> Option<f64> {
        if idx < 1 || idx as usize > self.data.len() {
            return None;
        }
        Some(self.data.remove((idx - 1) as usize))
    }

    /// Returns the length of the sequence.
    pub fn length(&self) -> i32 {
        self.data.len() as i32
    }

    /// Gets a reference to the value at the given 1-based index.
    pub fn at(&self, idx: i32) -> Option<&f64> {
        if idx < 1 || idx as usize > self.data.len() {
            return None;
        }
        Some(&self.data[(idx - 1) as usize])
    }

    /// Gets a mutable reference to the value at the given 1-based index.
    pub fn at_mut(&mut self, idx: i32) -> Option<&mut f64> {
        if idx < 1 || idx as usize > self.data.len() {
            return None;
        }
        Some(&mut self.data[(idx - 1) as usize])
    }

    /// Sets a value at the given 1-based index.
    pub fn set(&mut self, idx: i32, value: f64) {
        if idx < 1 || idx as usize > self.data.len() {
            panic!("Index {} out of bounds [1, {}]", idx, self.data.len());
        }
        self.data[(idx - 1) as usize] = value;
    }

    /// Clears the sequence.
    pub fn clear(&mut self) {
        self.data.clear();
    }
}

impl Default for TColStdHSequenceOfReal {
    fn default() -> Self {
        Self::new()
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_append_and_length() {
        let mut seq = TColStdHSequenceOfReal::new();
        assert_eq!(seq.length(), 0);

        seq.append(1.5);
        seq.append(2.5);
        seq.append(3.5);
        assert_eq!(seq.length(), 3);
    }

    #[test]
    fn test_at_1_based_indexing() {
        let mut seq = TColStdHSequenceOfReal::new();
        seq.append(1.1);
        seq.append(2.2);

        assert_eq!(seq.at(1), Some(&1.1));
        assert_eq!(seq.at(2), Some(&2.2));
        assert_eq!(seq.at(3), None);
    }

    #[test]
    fn test_insert_before() {
        let mut seq = TColStdHSequenceOfReal::new();
        seq.append(1.0);
        seq.append(3.0);
        seq.insert_before(2, 2.0);

        assert_eq!(seq.at(1), Some(&1.0));
        assert_eq!(seq.at(2), Some(&2.0));
        assert_eq!(seq.at(3), Some(&3.0));
    }

    #[test]
    fn test_remove() {
        let mut seq = TColStdHSequenceOfReal::new();
        seq.append(5.5);
        seq.append(10.5);

        let removed = seq.remove(1);
        assert_eq!(removed, Some(5.5));
        assert_eq!(seq.length(), 1);
    }

    #[test]
    fn test_set() {
        let mut seq = TColStdHSequenceOfReal::new();
        seq.append(1.0);
        seq.set(1, 9.9);

        assert_eq!(seq.at(1), Some(&9.9));
    }
}
