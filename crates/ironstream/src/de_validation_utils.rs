...
mod tests {
    ...
    /// Std-only stand-in for tempfile::NamedTempFile: a uniquely named file
    /// in the system temp directory, removed on drop.
    struct TempFile {
        path: PathBuf,
    }

    impl TempFile {
        fn create(tag: &str, content: &[u8]) -> std::io::Result<Self> {
    ...
    fn test_validate_file_for_reading_exists() {
        let file = TempFile::create("read", b"test content").expect("create temp file");
        let path = file.path_str();

        // Freshly created file exists and is not read-only, so validation succeeds.
    ...
    fn test_create_content_buffer_from_file() {
        let file = TempFile::create("buffer", b"test content").expect("create temp file");
        let path = file.path_str();
