// FILE: step_element_array1_of_measure_or_unspecified_value.rs
// occt: StepElement_Array1OfMeasureOrUnspecifiedValue

/// Rust port of OCCT's deprecated StepElement_Array1OfMeasureOrUnspecifiedValue.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepElementArray1OfMeasureOrUnspecifiedValue {
    data: Vec<f64>,
    lower: i32,
}

impl StepElementArray1OfMeasureOrUnspecifiedValue {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepElementArray1OfMeasureOrUnspecifiedValue::new(1, 5);
    ...
    fn test_array1_set_get() {
        let mut arr = StepElementArray1OfMeasureOrUnspecifiedValue::new(1, 3);
        arr.set_value(1, 1.5);
        arr.set_value(2, 2.5);
        arr.set_value(3, 3.5);

        assert_eq!(arr.value(1), Some(&1.5));
        assert_eq!(arr.value(2), Some(&2.5));
        assert_eq!(arr.value(3), Some(&3.5));
    }

    #[test]
    fn test_array1_bounds() {
        let arr = StepElementArray1OfMeasureOrUnspecifiedValue::new(1, 2);
    ...
    fn test_array1_from_vec() {
        ...
        let arr = StepElementArray1OfMeasureOrUnspecifiedValue::from_vec(1, values);

        assert_eq!(arr.length(), 3);
        assert_eq!(arr.value(1), Some(&0.1));
        assert_eq!(arr.value(2), Some(&0.2));
    }
}
