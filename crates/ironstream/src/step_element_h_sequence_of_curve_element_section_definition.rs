// FILE: step_element_h_sequence_of_curve_element_section_definition.rs
// occt: StepElement_HSequenceOfCurveElementSectionDefinition

/// Rust port of OCCT's deprecated StepElement_HSequenceOfCurveElementSectionDefinition.
/// HSequence is a heap-allocated dynamic sequence wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHSequenceOfCurveElementSectionDefinition {
    data: Vec<String>,
}

impl StepElementHSequenceOfCurveElementSectionDefinition {
...
impl Default for StepElementHSequenceOfCurveElementSectionDefinition {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_hsequence_creation() {
        let seq = StepElementHSequenceOfCurveElementSectionDefinition::new();
        assert_eq!(seq.length(), 0);
    }

    #[test]
    fn test_hsequence_append() {
        let mut seq = StepElementHSequenceOfCurveElementSectionDefinition::new();
        seq.append("def1".to_string());
        seq.append("def2".to_string());
        seq.append("def3".to_string());

        assert_eq!(seq.length(), 3);
        assert_eq!(seq.value(1), Some(&"def1".to_string()));
        assert_eq!(seq.value(3), Some(&"def3".to_string()));
    }
