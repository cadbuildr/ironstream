// FILE: step_element_h_array1_of_volume_element_purpose_member.rs
// occt: StepElement_HArray1OfVolumeElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_HArray1OfVolumeElementPurposeMember.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHArray1OfVolumeElementPurposeMember {
    data: Vec<String>,
    lower: i32,
}

impl StepElementHArray1OfVolumeElementPurposeMember {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepElementHArray1OfVolumeElementPurposeMember::new(1, 4);
    ...
    fn test_harray1_access() {
        let mut arr = StepElementHArray1OfVolumeElementPurposeMember::new(1, 2);
        arr.set_value(1, "vol_mem1".to_string());
        arr.set_value(2, "vol_mem2".to_string());

        assert_eq!(arr.value(1), Some(&"vol_mem1".to_string()));
        assert_eq!(arr.value(2), Some(&"vol_mem2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["vm1".to_string(), "vm2".to_string()];
        let arr = StepElementHArray1OfVolumeElementPurposeMember::from_vec(1, values);
