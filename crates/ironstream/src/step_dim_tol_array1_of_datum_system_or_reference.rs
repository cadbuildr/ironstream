// FILE: step_dim_tol_array1_of_datum_system_or_reference.rs
// occt: StepDimTol_Array1OfDatumSystemOrReference

/// Rust port of OCCT's deprecated StepDimTol_Array1OfDatumSystemOrReference.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepDimTolArray1OfDatumSystemOrReference {
    data: Vec<String>,
    lower: i32,
}

impl StepDimTolArray1OfDatumSystemOrReference {
...
mod tests {
    ...
    fn test_array1_bounds() {
        let arr = StepDimTolArray1OfDatumSystemOrReference::new(1, 5);
    ...
    fn test_array1_access() {
        let mut arr = StepDimTolArray1OfDatumSystemOrReference::new(1, 3);
        arr.set_value(1, "system1".to_string());
        arr.set_value(2, "ref1".to_string());
        arr.set_value(3, "system2".to_string());

        assert_eq!(arr.value(1), Some(&"system1".to_string()));
        assert_eq!(arr.value(2), Some(&"ref1".to_string()));
        assert_eq!(arr.value(3), Some(&"system2".to_string()));
    }

    #[test]
    fn test_array1_from_vec() {
        let values = vec!["a".to_string(), "b".to_string()];
        let arr = StepDimTolArray1OfDatumSystemOrReference::from_vec(3, values);

        assert_eq!(arr.lower_bound(), 3);
        assert_eq!(arr.upper_bound(), 4);
        assert_eq!(arr.value(3), Some(&"a".to_string()));
        assert_eq!(arr.value(4), Some(&"b".to_string()));
