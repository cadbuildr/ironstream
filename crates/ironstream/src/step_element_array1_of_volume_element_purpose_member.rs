// FILE: step_element_array1_of_volume_element_purpose_member.rs
// occt: StepElement_Array1OfVolumeElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_Array1OfVolumeElementPurposeMember.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepElementArray1OfVolumeElementPurposeMember {
    data: Vec<String>,
    lower: i32,
}

impl StepElementArray1OfVolumeElementPurposeMember {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepElementArray1OfVolumeElementPurposeMember::new(1, 4);
    ...
    fn test_array1_access() {
        let mut arr = StepElementArray1OfVolumeElementPurposeMember::new(1, 2);
        arr.set_value(1, "vol_member1".to_string());
        arr.set_value(2, "vol_member2".to_string());

        assert_eq!(arr.value(1), Some(&"vol_member1".to_string()));
        assert_eq!(arr.value(2), Some(&"vol_member2".to_string()));
    }

    #[test]
    fn test_array1_from_vec() {
        let values = vec!["vm1".to_string(), "vm2".to_string()];
        let arr = StepElementArray1OfVolumeElementPurposeMember::from_vec(1, values);

        assert_eq!(arr.length(), 2);
        assert_eq!(arr.value(1), Some(&"vm1".to_string()));
    }
}
