...
// occt: type of surface for face — Planar, Cylindrical, Spherical, Conical, Toroidal, BSpline, Offset
/// Surface kind carried by a parametric face.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FaceType {
    /// Flat plane — `Geom_Plane`.
    // occt: Geom_Plane (infinite planar surface)
    Planar,
...
// occt: BRepBuilderAPI_MakeFace input params — surface kind plus UV domain and tolerance
/// Input parameters for constructing a parametric face.
#[derive(Debug, Clone)]
pub struct FaceParams {
    // occt: surface type tag
    face_type: FaceType,
    // occt: U parameter range [u_min, u_max]
    u_range: (f64, f64),
    // occt: V parameter range [v_min, v_max]
    v_range: (f64, f64),
    // occt: BRepBuilderAPI_MakeFace tolerance for vertex/edge on surface
    tolerance: f64,
}

impl FaceParams {
    /// Construct with default UV range [0, 1] × [0, 1] and tolerance 1e-7.
    // occt: BRepBuilderAPI_MakeFace(surface) — default domain
    ...
    // occt: BRepBuilderAPI_MakeFace(surface, u1, u2, v1, v2, tol)
    ...
    // occt: BRepBuilderAPI_MakeFace::IsDone / Tolerance accessor
...
// occt: inner wire on a face — wraps BRep_Wire topology with edge count and orientation flag
/// A wire lying on a face, either the outer boundary or an interior hole.
#[derive(Debug, Clone)]
pub struct FaceWire {
    // occt: label / name of the wire for identification
    wire_label: String,
    // occt: number of edges in the wire
    nb_edges: usize,
    // occt: orientation — true for outer boundary, false for hole
    is_outer: bool,
}

impl FaceWire {
    /// Construct a new face wire.
    // occt: BRep_Builder::MakeWire + Add edge loop
...
// occt: BRepBuilderAPI_MakeFace — stub face builder that collects surface params and wires
...
pub struct BrepMakeFace {
    // occt: surface + UV-domain descriptor
    params: FaceParams,
    // occt: outer boundary wire (orientation FORWARD in OCCT)
    outer_wire: Option<FaceWire>,
    // occt: interior hole wires (orientation REVERSED in OCCT)
...
impl BrepMakeFace {
    ...
    // occt: BRepBuilderAPI_MakeFace(surface, tol)
    ...
    // occt: BRepBuilderAPI_MakeFace — count of inner wires via TopExp_Explorer
    pub fn nb_inner_wires(&self) -> usize {
        self.inner_wires.len()
    }

    /// Stub area: product of the U-span and V-span of the parameter domain.
    // occt: BRepGProp::SurfaceProperties stub — returns (u_max-u_min)*(v_max-v_min)
    ...
    // occt: BRep_Tool::Surface — surface type tag
    pub fn face_type(&self) -> FaceType {
