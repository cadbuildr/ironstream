// FILE: step_element_h_array1_of_curve_element_end_release_packet.rs
// occt: StepElement_HArray1OfCurveElementEndReleasePacket

/// Rust port of OCCT's deprecated StepElement_HArray1OfCurveElementEndReleasePacket.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHArray1OfCurveElementEndReleasePacket {
    data: Vec<String>,
    lower: i32,
}

impl StepElementHArray1OfCurveElementEndReleasePacket {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepElementHArray1OfCurveElementEndReleasePacket::new(1, 5);
    ...
    fn test_harray1_access() {
        let mut arr = StepElementHArray1OfCurveElementEndReleasePacket::new(1, 2);
        arr.set_value(1, "pkt1".to_string());
        arr.set_value(2, "pkt2".to_string());

        assert_eq!(arr.value(1), Some(&"pkt1".to_string()));
        assert_eq!(arr.value(2), Some(&"pkt2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["p1".to_string(), "p2".to_string()];
        let arr = StepElementHArray1OfCurveElementEndReleasePacket::from_vec(1, values);
