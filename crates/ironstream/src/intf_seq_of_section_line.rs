// FILE: intf_seq_of_section_line.rs
// occt: Intf_SeqOfSectionLine

/// Deprecated alias for NCollection_Sequence<Intf_SectionLine>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct IntfSeqOfSectionLine {
    items: Vec<u32>, // Placeholder for Intf_SectionLine (opaque type)
}

impl IntfSeqOfSectionLine {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for IntfSeqOfSectionLine {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = IntfSeqOfSectionLine::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = IntfSeqOfSectionLine::default();
        assert!(seq.is_empty());
    }
}
