// FILE: units_qts_sequence.rs
// occt: Units_QtsSequence

use std::fmt;

/// Placeholder for Units_Quantity handle (shared pointer)
#[derive(Clone, Debug)]
pub struct UnitsQuantityHandle {
    // In real implementation, would contain reference-counted pointer to Units_Quantity
}

/// Deprecated typedef: NCollection_Sequence<opencascade::handle<Units_Quantity>>
///
/// A sequence of Units_Quantity handles (Qts = Quantities).
...
#[derive(Clone, Debug)]
pub struct UnitsQtsSequence {
    items: Vec<UnitsQuantityHandle>,
    lower: i32,
}

impl UnitsQtsSequence {
    ...
    pub fn append(&mut self, elem: UnitsQuantityHandle) {
        self.items.push(elem);
    }

    /// Prepend an element to the sequence.
    pub fn prepend(&mut self, elem: UnitsQuantityHandle) {
        self.items.insert(0, elem);
    }

    /// Insert element at position (1-based OCCT indexing).
    pub fn insert(&mut self, index: i32, elem: UnitsQuantityHandle) {
    ...
    pub fn value(&self, index: i32) -> Option<&UnitsQuantityHandle> {
        if index < self.lower {
            return None;
        }
    ...
    pub fn change_value(&mut self, index: i32) -> Option<&mut UnitsQuantityHandle> {
        if index < self.lower {
            return None;
        }
    ...
    /// Get iterator.
    pub fn iter(&self) -> impl Iterator<Item = &UnitsQuantityHandle> {
        self.items.iter()
    }

    /// Get mutable iterator.
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut UnitsQuantityHandle> {
        self.items.iter_mut()
    }
...
impl Default for UnitsQtsSequence {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for UnitsQtsSequence {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "UnitsQtsSequence(size={})", self.size())
    }
}
...
mod tests {
    ...
    fn test_append_elements() {
        let mut seq = UnitsQtsSequence::new();
        let elem = UnitsQuantityHandle {};
    ...
    fn test_prepend_element() {
        let mut seq = UnitsQtsSequence::new();
        seq.append(UnitsQuantityHandle {});
        seq.append(UnitsQuantityHandle {});
        seq.prepend(UnitsQuantityHandle {});
    ...
    fn test_insert_element() {
        ...
        seq.append(UnitsQuantityHandle {});
        seq.append(UnitsQuantityHandle {});
        seq.insert(2, UnitsQuantityHandle {});
        assert_eq!(seq.size(), 3);
    }
