// FILE: l_prop_sequence_of_ci_type.rs
// occt: LProp_SequenceOfCIType

/// Deprecated alias for NCollection_Sequence<LProp_CIType>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct LPropSequenceOfCIType {
    items: Vec<u32>, // Placeholder for LProp_CIType (opaque type)
}

impl LPropSequenceOfCIType {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for LPropSequenceOfCIType {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = LPropSequenceOfCIType::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = LPropSequenceOfCIType::default();
        assert!(seq.is_empty());
    }
}
