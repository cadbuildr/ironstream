// FILE: iges_draw_tool_connect_point.rs
// occt: IGESDraw_ToolConnectPoint

/// Tool to work on a ConnectPoint
pub struct IgesDrawToolConnectPoint;

impl IgesDrawToolConnectPoint {
    pub fn new() -> Self {
        IgesDrawToolConnectPoint
    }

    pub fn read_own_params(&self) {}
    pub fn write_own_params(&self) {}
    pub fn own_shared(&self) {}
    pub fn dir_checker(&self) {}
    pub fn own_check(&self) {}
    pub fn own_copy(&self) {}
    pub fn own_dump(&self) {}
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_new() {
        let tool = IgesDrawToolConnectPoint::new();
        assert_eq!(std::mem::size_of_val(&tool), 0);
    }
}
