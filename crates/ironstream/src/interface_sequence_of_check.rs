// FILE: interface_sequence_of_check.rs
// occt: Interface_SequenceOfCheck

/// Deprecated alias for NCollection_Sequence<opencascade::handle<Interface_Check>>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct InterfaceSequenceOfCheck {
    items: Vec<u32>, // Placeholder for Interface_Check (opaque type)
}

impl InterfaceSequenceOfCheck {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for InterfaceSequenceOfCheck {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = InterfaceSequenceOfCheck::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = InterfaceSequenceOfCheck::default();
        assert!(seq.is_empty());
    }
}
