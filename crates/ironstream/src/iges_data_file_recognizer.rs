...
impl FileRecognizer {
    ...
    pub fn add(&self, recognizer: FileRecognizer) {
        ...
        // Chain to the next recognizer: clone its shared handles so the
        // borrow on self.next can be released before recursing.
        let next_owned = {
            let next = next_opt.as_ref().unwrap();
            FileRecognizer {
                result: Rc::clone(&next.result),
                next: Rc::clone(&next.next),
            }
        };
        drop(next_opt);
    ...
    /// In OCCT, Eval is deferred (pure virtual); the base class leaves the
    /// recorded result untouched.
    pub fn eval(&self, _type_num: i32, _form_num: i32) {}
