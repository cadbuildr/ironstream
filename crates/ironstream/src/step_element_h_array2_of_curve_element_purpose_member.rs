// FILE: step_element_h_array2_of_curve_element_purpose_member.rs
// occt: StepElement_HArray2OfCurveElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_HArray2OfCurveElementPurposeMember.
/// A 2D 1-based array wrapping a Vec in row-major order, mimicking NCollection_HArray2 semantics.
#[derive(Clone, Debug)]
pub struct StepElementHArray2OfCurveElementPurposeMember {
    data: Vec<String>,
    row_lower: i32,
    row_upper: i32,
    col_lower: i32,
...
impl StepElementHArray2OfCurveElementPurposeMember {
...
mod tests {
    ...
    fn test_harray2_creation() {
        let arr = StepElementHArray2OfCurveElementPurposeMember::new(1, 2, 1, 3);
    ...
    fn test_harray2_access() {
        let mut arr = StepElementHArray2OfCurveElementPurposeMember::new(1, 2, 1, 2);
        arr.set_value(1, 1, "h11".to_string());
        arr.set_value(2, 2, "h22".to_string());

        assert_eq!(arr.value(1, 1), Some(&"h11".to_string()));
        assert_eq!(arr.value(2, 2), Some(&"h22".to_string()));
    }

    #[test]
    fn test_harray2_bounds() {
        let arr = StepElementHArray2OfCurveElementPurposeMember::new(1, 2, 1, 2);
