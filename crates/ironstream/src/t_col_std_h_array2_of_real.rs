// FILE: t_col_std_h_array2_of_real.rs
// occt: TColStd_HArray2OfReal

/// TColStd_HArray2OfReal is a deprecated alias for a handle (heap-allocated) 2D 1-based array of real numbers.
/// This is a Rust port implementing OCCT's 1-based indexing semantics.
pub struct TColStdHArray2OfReal {
    data: Vec<f64>,
    row_lower: i32,
    row_upper: i32,
    col_lower: i32,
...
impl TColStdHArray2OfReal {
    ...
    pub fn at(&self, row: i32, col: i32) -> Option<&f64> {
        if row < self.row_lower || row > self.row_upper || col < self.col_lower
            || col > self.col_upper
        {
            return None;
    ...
    pub fn at_mut(&mut self, row: i32, col: i32) -> Option<&mut f64> {
        if row < self.row_lower || row > self.row_upper || col < self.col_lower
            || col > self.col_upper
        {
            return None;
...
mod tests {
    ...
    fn test_creation_and_bounds() {
        let arr = TColStdHArray2OfReal::new(1, 3, 1, 2);
    ...
    #[test]
    fn test_set_and_get() {
        let mut arr = TColStdHArray2OfReal::new(1, 2, 1, 2);
        arr.set(1, 1, 1.5);
        arr.set(1, 2, 2.5);
        arr.set(2, 1, 3.5);
        arr.set(2, 2, 4.5);

        assert_eq!(arr.at(1, 1), Some(&1.5));
        assert_eq!(arr.at(1, 2), Some(&2.5));
        assert_eq!(arr.at(2, 1), Some(&3.5));
        assert_eq!(arr.at(2, 2), Some(&4.5));
    }

    #[test]
    fn test_at_out_of_bounds() {
        let arr = TColStdHArray2OfReal::new(1, 2, 1, 2);
