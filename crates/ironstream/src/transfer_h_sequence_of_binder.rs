// FILE: transfer_h_sequence_of_binder.rs
// occt: Transfer_HSequenceOfBinder

use std::fmt;

/// Placeholder for Transfer_Binder handle (shared pointer)
#[derive(Clone, Debug)]
pub struct TransferBinderHandle {
    // In real implementation, would contain reference-counted pointer to Transfer_Binder
}

/// Deprecated typedef: NCollection_HSequence<opencascade::handle<Transfer_Binder>>
///
/// A handle-based sequence of Transfer_Binder objects.
...
#[derive(Clone, Debug)]
pub struct TransferHSequenceOfBinder {
    items: Vec<TransferBinderHandle>,
    lower: i32,
}

impl TransferHSequenceOfBinder {
    /// Create an empty sequence.
    pub fn new() -> Self {
        TransferHSequenceOfBinder {
            items: Vec::new(),
            lower: 1,
        }
    }
    ...
    pub fn append(&mut self, elem: TransferBinderHandle) {
        self.items.push(elem);
    }

    /// Insert element at position (1-based OCCT indexing).
    pub fn insert(&mut self, index: i32, elem: TransferBinderHandle) {
    ...
    pub fn value(&self, index: i32) -> Option<&TransferBinderHandle> {
        if index < self.lower {
            return None;
        }
    ...
    pub fn change_value(&mut self, index: i32) -> Option<&mut TransferBinderHandle> {
        if index < self.lower {
            return None;
        }
    ...
    /// Get iterator.
    pub fn iter(&self) -> impl Iterator<Item = &TransferBinderHandle> {
        self.items.iter()
    }

    /// Get mutable iterator.
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut TransferBinderHandle> {
        self.items.iter_mut()
    }
...
impl Default for TransferHSequenceOfBinder {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for TransferHSequenceOfBinder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "TransferHSequenceOfBinder(size={})", self.size())
    }
}
...
mod tests {
    ...
    fn test_create_empty_sequence() {
        let seq = TransferHSequenceOfBinder::new();
        assert!(seq.is_empty());
    ...
    fn test_append_elements() {
        let mut seq = TransferHSequenceOfBinder::new();
        let elem = TransferBinderHandle {};
    ...
    fn test_occt_1based_indexing() {
        let mut seq = TransferHSequenceOfBinder::new();
        seq.append(TransferBinderHandle {});
        seq.append(TransferBinderHandle {});
        seq.append(TransferBinderHandle {});
    ...
    fn test_insert_element() {
        ...
        seq.append(TransferBinderHandle {});
        seq.append(TransferBinderHandle {});
        seq.insert(2, TransferBinderHandle {});
