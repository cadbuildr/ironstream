// FILE: interface_h_sequence_of_check.rs
// occt: Interface_HSequenceOfCheck

/// Deprecated alias for NCollection_HSequence<opencascade::handle<Interface_Check>>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct InterfaceHSequenceOfCheck {
    items: Vec<u32>, // Placeholder for Interface_Check (opaque type)
}

impl InterfaceHSequenceOfCheck {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for InterfaceHSequenceOfCheck {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = InterfaceHSequenceOfCheck::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = InterfaceHSequenceOfCheck::default();
        assert!(seq.is_empty());
    }
}
