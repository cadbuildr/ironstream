// FILE: step_fea_sequence_of_curve3d_element_property.rs
// occt: StepFEA_SequenceOfCurve3dElementProperty

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Sequence<StepFEA_Curve3dElementProperty>.
/// Provides 1-based indexing semantics compatible with OCCT's Sequence container.
pub struct StepFEASequenceOfCurve3dElementProperty {
    data: Vec<Option<String>>, // Using String as placeholder for StepFEA_Curve3dElementProperty handle
    lower: usize,
}

impl StepFEASequenceOfCurve3dElementProperty {
...
impl Default for StepFEASequenceOfCurve3dElementProperty {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_new_sequence_is_empty() {
        let seq = StepFEASequenceOfCurve3dElementProperty::new();
        assert!(seq.is_empty());
        assert_eq!(seq.len(), 0);
        assert_eq!(seq.lower(), 1);
        assert_eq!(seq.upper(), 0); // Empty: upper < lower
    }

    #[test]
    fn test_append_and_length() {
        let mut seq = StepFEASequenceOfCurve3dElementProperty::new();
    ...
    fn test_one_based_indexing() {
        ...
        assert_eq!(seq.value(3), Some(&Some("third".to_string())));
    ...
    #[test]
    fn test_default() {
        let seq = StepFEASequenceOfCurve3dElementProperty::default();
        assert!(seq.is_empty());
        assert_eq!(seq.lower(), 1);
    }
}
