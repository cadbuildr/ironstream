// FILE: loc_ope_sequence_of_circ.rs
// occt: LocOpe_SequenceOfCirc

/// Deprecated alias for NCollection_Sequence<gp_Circ>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct LocOpeSequenceOfCirc {
    items: Vec<u32>, // Placeholder for gp_Circ (opaque type)
}

impl LocOpeSequenceOfCirc {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for LocOpeSequenceOfCirc {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = LocOpeSequenceOfCirc::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = LocOpeSequenceOfCirc::default();
        assert!(seq.is_empty());
    }
}
