// FILE: step_element_h_sequence_of_element_material.rs
// occt: StepElement_HSequenceOfElementMaterial

/// Rust port of OCCT's deprecated StepElement_HSequenceOfElementMaterial.
/// HSequence is a heap-allocated dynamic sequence wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHSequenceOfElementMaterial {
    data: Vec<String>,
}

impl StepElementHSequenceOfElementMaterial {
...
impl Default for StepElementHSequenceOfElementMaterial {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_hsequence_creation() {
        let seq = StepElementHSequenceOfElementMaterial::new();
        assert_eq!(seq.length(), 0);
    }

    #[test]
    fn test_hsequence_append() {
        let mut seq = StepElementHSequenceOfElementMaterial::new();
        seq.append("material1".to_string());
        seq.append("material2".to_string());

        assert_eq!(seq.length(), 2);
        assert_eq!(seq.value(1), Some(&"material1".to_string()));
        assert_eq!(seq.value(2), Some(&"material2".to_string()));
    }
    ...
    fn test_hsequence_set_value() {
        ...
        seq.set_value(1, "updated".to_string());

        assert_eq!(seq.value(1), Some(&"updated".to_string()));
    }

    #[test]
    fn test_hsequence_clear() {
        let mut seq = StepElementHSequenceOfElementMaterial::new();
        seq.append("m1".to_string());
        seq.append("m2".to_string());
        seq.clear();
