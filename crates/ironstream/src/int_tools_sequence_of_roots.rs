// FILE: int_tools_sequence_of_roots.rs
// occt: IntTools_SequenceOfRoots

/// Deprecated alias for NCollection_Sequence<IntTools_Root>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct IntToolsSequenceOfRoots {
    items: Vec<u32>, // Placeholder for IntTools_Root (opaque type)
}

impl IntToolsSequenceOfRoots {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for IntToolsSequenceOfRoots {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = IntToolsSequenceOfRoots::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = IntToolsSequenceOfRoots::default();
        assert!(seq.is_empty());
    }
}
