// FILE: moni_tool_h_sequence_of_element.rs
// occt: MoniTool_HSequenceOfElement
...
/// MoniTool_SequenceOfElement is a sequence of elements.
pub type MoniToolSequenceOfElement = Vec<MoniToolElementHandle>;

/// NCollection_HSequence is a handle-based sequence.
/// MoniTool_HSequenceOfElement is a reference-counted wrapper around MoniTool_SequenceOfElement.
pub type MoniToolHSequenceOfElement = Rc<RefCell<MoniToolSequenceOfElement>>;
...
mod tests {
    ...
    #[test]
    fn test_h_sequence_handle() {
        let seq: MoniToolSequenceOfElement = Vec::new();
        let h_seq: MoniToolHSequenceOfElement = Rc::new(RefCell::new(seq));

        {
            let mut sequence = h_seq.borrow_mut();
            let elem = Rc::new(RefCell::new(MoniToolElement::new(
                10,
                "Test".to_string(),
                "Data".to_string(),
        ...
        {
            let sequence = h_seq.borrow();
            assert_eq!(sequence.len(), 1);
            assert_eq!(sequence[0].borrow().id(), 10);
        }
    }
}
