// FILE: step_element_array1_of_curve_element_section_definition.rs
// occt: StepElement_Array1OfCurveElementSectionDefinition

/// Rust port of OCCT's deprecated StepElement_Array1OfCurveElementSectionDefinition.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepElementArray1OfCurveElementSectionDefinition {
    data: Vec<String>,
    lower: i32,
}

impl StepElementArray1OfCurveElementSectionDefinition {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepElementArray1OfCurveElementSectionDefinition::new(1, 4);
    ...
    fn test_array1_access() {
        let mut arr = StepElementArray1OfCurveElementSectionDefinition::new(1, 2);
        arr.set_value(1, "def1".to_string());
        arr.set_value(2, "def2".to_string());

        assert_eq!(arr.value(1), Some(&"def1".to_string()));
        assert_eq!(arr.value(2), Some(&"def2".to_string()));
    }

    #[test]
    fn test_array1_from_vec() {
        let values = vec!["section1".to_string(), "section2".to_string(), "section3".to_string()];
        let arr = StepElementArray1OfCurveElementSectionDefinition::from_vec(1, values);

        assert_eq!(arr.length(), 3);
        assert_eq!(arr.value(1), Some(&"section1".to_string()));
        assert_eq!(arr.value(3), Some(&"section3".to_string()));
    }
}
