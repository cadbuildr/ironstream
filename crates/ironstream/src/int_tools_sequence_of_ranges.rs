// FILE: int_tools_sequence_of_ranges.rs
// occt: IntTools_SequenceOfRanges

/// Deprecated alias for NCollection_Sequence<IntTools_Range>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct IntToolsSequenceOfRanges {
    items: Vec<u32>, // Placeholder for IntTools_Range (opaque type)
}

impl IntToolsSequenceOfRanges {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for IntToolsSequenceOfRanges {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = IntToolsSequenceOfRanges::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = IntToolsSequenceOfRanges::default();
        assert!(seq.is_empty());
    }
}
