// FILE: t_col_std_h_sequence_of_h_extended_string.rs
// occt: TColStd_HSequenceOfHExtendedString

/// TColStd_HSequenceOfHExtendedString is a deprecated alias for a handle sequence of handle extended strings.
/// This is a Rust port implementing OCCT's sequence semantics (1-based indexing).
pub struct TColStdHSequenceOfHExtendedString {
    data: Vec<Option<String>>,
}

impl TColStdHSequenceOfHExtendedString {
...
impl Default for TColStdHSequenceOfHExtendedString {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_append_and_length() {
        let mut seq = TColStdHSequenceOfHExtendedString::new();
        assert_eq!(seq.length(), 0);

        seq.append(Some("unicode".to_string()));
        seq.append(None);
