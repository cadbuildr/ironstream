// FILE: step_fea_h_array1_of_curve_element_end_release.rs
// occt: StepFEA_HArray1OfCurveElementEndRelease

/// Rust port of OCCT's deprecated StepFEA_HArray1OfCurveElementEndRelease.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepFEAHArray1OfCurveElementEndRelease {
    data: Vec<String>,
    lower: i32,
}

impl StepFEAHArray1OfCurveElementEndRelease {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepFEAHArray1OfCurveElementEndRelease::new(1, 4);
    ...
    fn test_harray1_access() {
        let mut arr = StepFEAHArray1OfCurveElementEndRelease::new(1, 2);
        arr.set_value(1, "rel1".to_string());
        arr.set_value(2, "rel2".to_string());

        assert_eq!(arr.value(1), Some(&"rel1".to_string()));
        assert_eq!(arr.value(2), Some(&"rel2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["r1".to_string(), "r2".to_string(), "r3".to_string()];
        let arr = StepFEAHArray1OfCurveElementEndRelease::from_vec(1, values);

        assert_eq!(arr.length(), 3);
        assert_eq!(arr.value(2), Some(&"r2".to_string()));
    }
}
