// FILE: int_tools_sequence_of_curves.rs
// occt: IntTools_SequenceOfCurves

/// Deprecated alias for NCollection_Sequence<IntTools_Curve>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct IntToolsSequenceOfCurves {
    items: Vec<u32>, // Placeholder for IntTools_Curve (opaque type)
}

impl IntToolsSequenceOfCurves {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for IntToolsSequenceOfCurves {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = IntToolsSequenceOfCurves::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = IntToolsSequenceOfCurves::default();
        assert!(seq.is_empty());
    }
}
