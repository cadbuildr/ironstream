// FILE: step_element_h_array1_of_h_sequence_of_surface_element_purpose_member.rs
// occt: StepElement_HArray1OfHSequenceOfSurfaceElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_HArray1OfHSequenceOfSurfaceElementPurposeMember.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHArray1OfHSequenceOfSurfaceElementPurposeMember {
    data: Vec<String>,
    lower: i32,
}

impl StepElementHArray1OfHSequenceOfSurfaceElementPurposeMember {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepElementHArray1OfHSequenceOfSurfaceElementPurposeMember::new(1, 4);
    ...
    fn test_harray1_access() {
        let mut arr = StepElementHArray1OfHSequenceOfSurfaceElementPurposeMember::new(1, 2);
        arr.set_value(1, "surf_mem1".to_string());
        arr.set_value(2, "surf_mem2".to_string());

        assert_eq!(arr.value(1), Some(&"surf_mem1".to_string()));
        assert_eq!(arr.value(2), Some(&"surf_mem2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["sm1".to_string(), "sm2".to_string()];
        let arr = StepElementHArray1OfHSequenceOfSurfaceElementPurposeMember::from_vec(1, values);
