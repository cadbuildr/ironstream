// FILE: step_geom_array1_of_surface_boundary.rs
// occt: StepGeom_Array1OfSurfaceBoundary

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Array1<StepGeom_SurfaceBoundary>.
/// Provides 1-based indexing semantics with fixed bounds (Lower/Upper).
pub struct StepGeomArray1OfSurfaceBoundary {
    data: Vec<Option<String>>,
    lower: usize,
}

impl StepGeomArray1OfSurfaceBoundary {
...
mod tests {
    ...
    fn test_new_array_with_bounds() {
        let arr = StepGeomArray1OfSurfaceBoundary::new(1, 5);
    ...
    fn test_one_based_indexing() {
        let mut arr = StepGeomArray1OfSurfaceBoundary::new(1, 3);
        arr.set_value(1, Some("sb1".to_string()));
        arr.set_value(2, Some("sb2".to_string()));
        arr.set_value(3, Some("sb3".to_string()));

        assert_eq!(arr.value(1), Some(&Some("sb1".to_string())));
        assert_eq!(arr.value(2), Some(&Some("sb2".to_string())));
        assert_eq!(arr.value(3), Some(&Some("sb3".to_string())));
    }

    #[test]
    fn test_bounds_checking() {
        let arr = StepGeomArray1OfSurfaceBoundary::new(1, 3);
