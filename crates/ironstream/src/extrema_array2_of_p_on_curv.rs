// FILE: extrema_array2_of_p_on_curv.rs
// occt: Extrema_Array2OfPOnCurv

#[derive(Clone, Debug)]
pub struct POnCurv {}

#[derive(Clone, Debug)]
pub struct Array2OfPOnCurv {
    items: Vec<Vec<POnCurv>>,
    lower1: usize,
    lower2: usize,
}

impl Array2OfPOnCurv {
    pub fn new(lower1: usize, upper1: usize, lower2: usize, upper2: usize) -> Self {
        ...
        Array2OfPOnCurv {
            items: vec![vec![POnCurv {}; size2]; size1],
            lower1,
            lower2,
        }
    }

    pub fn at(&self, i: usize, j: usize) -> Option<&POnCurv> {
        let i_idx = i.checked_sub(self.lower1)?;
        let j_idx = j.checked_sub(self.lower2)?;
        self.items.get(i_idx)?.get(j_idx)
    }

    pub fn set(&mut self, i: usize, j: usize, val: POnCurv) -> bool {
...
mod tests {
    ...
    fn test_array2_creation() {
        let arr = Array2OfPOnCurv::new(1, 3, 1, 2);
        assert_eq!(arr.lower1(), 1);
        assert_eq!(arr.lower2(), 1);
    }
    ...
    fn test_array2_access() {
        ...
        assert!(arr.at(3, 2).is_some());
        assert!(arr.at(4, 1).is_none());
    }

    #[test]
    fn test_array2_set() {
        let mut arr = Array2OfPOnCurv::new(1, 2, 1, 2);
        assert!(arr.set(1, 1, POnCurv {}));
        assert!(!arr.set(3, 3, POnCurv {}));
    }
}
