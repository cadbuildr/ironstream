// FILE: transfer_b_rep_h_sequence_of_transfer_result_info.rs
// occt: TransferBRep_HSequenceOfTransferResultInfo

use std::fmt;

/// Placeholder for TransferBRep_TransferResultInfo handle (shared pointer)
#[derive(Clone, Debug)]
pub struct TransferBRepTransferResultInfoHandle {
    // In real implementation, would contain reference-counted pointer to TransferBRep_TransferResultInfo
}

/// Deprecated typedef: NCollection_HSequence<opencascade::handle<TransferBRep_TransferResultInfo>>
///
/// A handle-based sequence of TransferResultInfo objects.
...
#[derive(Clone, Debug)]
pub struct TransferBRepHSequenceOfTransferResultInfo {
    items: Vec<TransferBRepTransferResultInfoHandle>,
    lower: i32,
}

impl TransferBRepHSequenceOfTransferResultInfo {
    ...
    pub fn append(&mut self, elem: TransferBRepTransferResultInfoHandle) {
        self.items.push(elem);
    }

    /// Insert element at position (1-based OCCT indexing).
    pub fn insert(&mut self, index: i32, elem: TransferBRepTransferResultInfoHandle) {
    ...
    pub fn value(&self, index: i32) -> Option<&TransferBRepTransferResultInfoHandle> {
        if index < self.lower {
            return None;
        }
    ...
    pub fn change_value(
        &mut self,
        index: i32,
    ) -> Option<&mut TransferBRepTransferResultInfoHandle> {
        if index < self.lower {
            return None;
        }
    ...
    /// Get iterator.
    pub fn iter(&self) -> impl Iterator<Item = &TransferBRepTransferResultInfoHandle> {
        self.items.iter()
    }

    /// Get mutable iterator.
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut TransferBRepTransferResultInfoHandle> {
        self.items.iter_mut()
    }
...
impl Default for TransferBRepHSequenceOfTransferResultInfo {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for TransferBRepHSequenceOfTransferResultInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "TransferBRepHSequenceOfTransferResultInfo(size={})",
            self.size()
        )
    }
}
...
mod tests {
    ...
    fn test_create_empty_sequence() {
        let seq = TransferBRepHSequenceOfTransferResultInfo::new();
        assert!(seq.is_empty());
    ...
    fn test_append_elements() {
        let mut seq = TransferBRepHSequenceOfTransferResultInfo::new();
        let elem = TransferBRepTransferResultInfoHandle {};
    ...
    fn test_occt_1based_indexing() {
        let mut seq = TransferBRepHSequenceOfTransferResultInfo::new();
        seq.append(TransferBRepTransferResultInfoHandle {});
        seq.append(TransferBRepTransferResultInfoHandle {});
        seq.append(TransferBRepTransferResultInfoHandle {});
    ...
    fn test_insert_element() {
        ...
        seq.append(TransferBRepTransferResultInfoHandle {});
        seq.append(TransferBRepTransferResultInfoHandle {});
        seq.insert(2, TransferBRepTransferResultInfoHandle {});
