// FILE: step_fea_sequence_of_element_representation.rs
// occt: StepFEA_SequenceOfElementRepresentation

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Sequence<StepFEA_ElementRepresentation>.
/// Provides 1-based indexing semantics compatible with OCCT's Sequence container.
pub struct StepFEASequenceOfElementRepresentation {
    data: Vec<Option<String>>, // Using String as placeholder for StepFEA_ElementRepresentation handle
    lower: usize,
}

impl StepFEASequenceOfElementRepresentation {
...
impl Default for StepFEASequenceOfElementRepresentation {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_new_sequence_is_empty() {
        let seq = StepFEASequenceOfElementRepresentation::new();
        assert!(seq.is_empty());
        assert_eq!(seq.len(), 0);
    ...
    fn test_append_and_length() {
        let mut seq = StepFEASequenceOfElementRepresentation::new();
        seq.append(Some("rep1".to_string()));
        seq.append(Some("rep2".to_string()));
    ...
    fn test_one_based_indexing() {
        let mut seq = StepFEASequenceOfElementRepresentation::new();
        seq.append(Some("e1".to_string()));
        seq.append(Some("e2".to_string()));
        seq.append(Some("e3".to_string()));

        assert_eq!(seq.value(1), Some(&Some("e1".to_string())));
        assert_eq!(seq.value(2), Some(&Some("e2".to_string())));
        assert_eq!(seq.value(3), Some(&Some("e3".to_string())));
