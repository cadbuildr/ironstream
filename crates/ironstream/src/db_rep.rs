...
static SHAPE_STORAGE_CELL: OnceLock<Mutex<HashMap<String, TopodsShape>>> = OnceLock::new();
static PARAMS_CELL: OnceLock<Mutex<DbrepParams>> = OnceLock::new();

fn shape_storage() -> &'static Mutex<HashMap<String, TopodsShape>> {
    SHAPE_STORAGE_CELL.get_or_init(|| Mutex::new(HashMap::new()))
}

fn params_storage() -> &'static Mutex<DbrepParams> {
    PARAMS_CELL.get_or_init(|| {
        Mutex::new(DbrepParams {
            with_hlr: false,
            with_rg1: true,
            with_rgn: true,
            with_hid: false,
