// FILE: step_fea_array1_of_node_representation.rs
// occt: StepFEA_Array1OfNodeRepresentation

/// Rust port of OCCT's deprecated StepFEA_Array1OfNodeRepresentation.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepFEAArray1OfNodeRepresentation {
    data: Vec<String>,
    lower: i32,
}

impl StepFEAArray1OfNodeRepresentation {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepFEAArray1OfNodeRepresentation::new(1, 5);
    ...
    fn test_array1_access() {
        let mut arr = StepFEAArray1OfNodeRepresentation::new(1, 2);
        arr.set_value(1, "node_repr1".to_string());
        arr.set_value(2, "node_repr2".to_string());

        assert_eq!(arr.value(1), Some(&"node_repr1".to_string()));
        assert_eq!(arr.value(2), Some(&"node_repr2".to_string()));
    }

    #[test]
    fn test_array1_from_vec() {
        let values = vec!["nr1".to_string(), "nr2".to_string(), "nr3".to_string()];
        let arr = StepFEAArray1OfNodeRepresentation::from_vec(1, values);

        assert_eq!(arr.length(), 3);
        assert_eq!(arr.value(2), Some(&"nr2".to_string()));
    }
}
