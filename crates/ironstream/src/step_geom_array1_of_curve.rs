// FILE: step_geom_array1_of_curve.rs
// occt: StepGeom_Array1OfCurve

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Array1<StepGeom_Curve>.
/// Provides 1-based indexing semantics with fixed bounds (Lower/Upper).
pub struct StepGeomArray1OfCurve {
    data: Vec<Option<String>>,
    lower: usize,
}

impl StepGeomArray1OfCurve {
...
mod tests {
    ...
    fn test_new_array_with_bounds() {
        let arr = StepGeomArray1OfCurve::new(1, 5);
    ...
    fn test_one_based_indexing() {
        let mut arr = StepGeomArray1OfCurve::new(1, 3);
        arr.set_value(1, Some("curve1".to_string()));
        arr.set_value(2, Some("curve2".to_string()));
        arr.set_value(3, Some("curve3".to_string()));

        assert_eq!(arr.value(1), Some(&Some("curve1".to_string())));
        assert_eq!(arr.value(2), Some(&Some("curve2".to_string())));
        assert_eq!(arr.value(3), Some(&Some("curve3".to_string())));
    }

    #[test]
    fn test_bounds_checking() {
        let arr = StepGeomArray1OfCurve::new(1, 3);
