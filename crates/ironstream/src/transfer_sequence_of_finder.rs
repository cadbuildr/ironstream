// FILE: transfer_sequence_of_finder.rs
// occt: Transfer_SequenceOfFinder

use std::fmt;

/// Placeholder for Transfer_Finder handle (shared pointer)
#[derive(Clone, Debug)]
pub struct TransferFinderSeqHandle {
    // In real implementation, would contain reference-counted pointer to Transfer_Finder
}

/// Deprecated typedef: NCollection_Sequence<opencascade::handle<Transfer_Finder>>
///
/// A sequence of Transfer_Finder handles.
...
#[derive(Clone, Debug)]
pub struct TransferSequenceOfFinder {
    items: Vec<TransferFinderSeqHandle>,
    lower: i32,
}

impl TransferSequenceOfFinder {
    /// Create an empty sequence.
    pub fn new() -> Self {
        TransferSequenceOfFinder {
            items: Vec::new(),
            lower: 1,
        }
    }
    ...
    pub fn append(&mut self, elem: TransferFinderSeqHandle) {
        self.items.push(elem);
    }

    /// Prepend an element to the sequence.
    pub fn prepend(&mut self, elem: TransferFinderSeqHandle) {
        self.items.insert(0, elem);
    }

    /// Insert element at position (1-based OCCT indexing).
    pub fn insert(&mut self, index: i32, elem: TransferFinderSeqHandle) {
    ...
    pub fn value(&self, index: i32) -> Option<&TransferFinderSeqHandle> {
        if index < self.lower {
            return None;
        }
    ...
    pub fn change_value(&mut self, index: i32) -> Option<&mut TransferFinderSeqHandle> {
        if index < self.lower {
            return None;
        }
    ...
    /// Get iterator.
    pub fn iter(&self) -> impl Iterator<Item = &TransferFinderSeqHandle> {
        self.items.iter()
    }

    /// Get mutable iterator.
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut TransferFinderSeqHandle> {
        self.items.iter_mut()
    }
...
impl Default for TransferSequenceOfFinder {
    fn default() -> Self {
        Self::new()
    }
}

impl fmt::Display for TransferSequenceOfFinder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "TransferSequenceOfFinder(size={})", self.size())
    }
}
...
mod tests {
    ...
    fn test_create_empty_sequence() {
        let seq = TransferSequenceOfFinder::new();
        assert!(seq.is_empty());
    ...
    fn test_append_elements() {
        let mut seq = TransferSequenceOfFinder::new();
        let elem = TransferFinderSeqHandle {};
    ...
    fn test_prepend_element() {
        let mut seq = TransferSequenceOfFinder::new();
        seq.append(TransferFinderSeqHandle {});
        seq.append(TransferFinderSeqHandle {});
        seq.prepend(TransferFinderSeqHandle {});
    ...
    fn test_insert_element() {
        ...
        seq.append(TransferFinderSeqHandle {});
        seq.append(TransferFinderSeqHandle {});
        seq.insert(2, TransferFinderSeqHandle {});
        assert_eq!(seq.size(), 3);
    }
