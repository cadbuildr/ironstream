// FILE: step_fea_sequence_of_node_representation.rs
// occt: StepFEA_SequenceOfNodeRepresentation

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Sequence<StepFEA_NodeRepresentation>.
/// Provides 1-based indexing semantics compatible with OCCT's Sequence container.
pub struct StepFEASequenceOfNodeRepresentation {
    data: Vec<Option<String>>, // Using String as placeholder for StepFEA_NodeRepresentation handle
    lower: usize,
}

impl StepFEASequenceOfNodeRepresentation {
...
impl Default for StepFEASequenceOfNodeRepresentation {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_new_sequence_is_empty() {
        let seq = StepFEASequenceOfNodeRepresentation::new();
        assert!(seq.is_empty());
        assert_eq!(seq.len(), 0);
    ...
    fn test_append_and_length() {
        let mut seq = StepFEASequenceOfNodeRepresentation::new();
        seq.append(Some("node1".to_string()));
        seq.append(Some("node2".to_string()));
    ...
    fn test_one_based_indexing() {
        let mut seq = StepFEASequenceOfNodeRepresentation::new();
        seq.append(Some("n1".to_string()));
        seq.append(Some("n2".to_string()));
        seq.append(Some("n3".to_string()));

        assert_eq!(seq.value(1), Some(&Some("n1".to_string())));
        assert_eq!(seq.value(2), Some(&Some("n2".to_string())));
        assert_eq!(seq.value(3), Some(&Some("n3".to_string())));
