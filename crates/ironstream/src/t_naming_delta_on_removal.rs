...
/// Local helper mirroring TNaming_Evolution.
...
/// Local helper mirroring TopoDS_Shape (external plumbing).
/// A default-constructed shape is null, like TopoDS_Shape().
...
impl TopodsShape {
    ...
    pub fn new() -> Self {
        TopodsShape { id: 0, null: true }
    }

    /// Creates a non-null shape identified by `id`.
    pub fn with_id(id: u64) -> Self {
        TopodsShape { id, null: false }
    }
...
/// Local helper mirroring TNaming_NamedShape (external plumbing):
/// an attribute holding (old shape, new shape) pairs plus an evolution.
#[derive(Clone, Debug)]
pub struct TNamingNamedShape {
    evolution: TNamingEvolution,
    pairs: Vec<(TopodsShape, TopodsShape)>,
}
...
impl TNamingNamedShape {
    ...
    pub fn add_shape(&mut self, old_shape: TopodsShape, new_shape: TopodsShape) {
        self.pairs.push((old_shape, new_shape));
    }
    ...
    pub fn pairs(&self) -> &[(TopodsShape, TopodsShape)] {
        &self.pairs
    }
}

/// Local helper mirroring TNaming_DeltaOnModification: captures the state
/// (non-null old and new shapes) of a named shape so it can be re-applied.
#[derive(Clone, Debug)]
...
impl TNamingDeltaOnModification {
    /// Captures the delta from a named shape. Null shapes are skipped,
    /// as OCCT skips null old/new shapes when iterating the attribute.
    pub fn new(named_shape: &TNamingNamedShape) -> Self {
        let mut old_shapes = Vec::new();
        let mut new_shapes = Vec::new();
        for (old, new) in named_shape.pairs() {
            if !old.is_null() {
                old_shapes.push(old.clone());
            }
            if !new.is_null() {
                new_shapes.push(new.clone());
            }
        }
        TNamingDeltaOnModification {
            evolution: named_shape.evolution(),
            old_shapes,
            new_shapes,
        }
    }

    /// Applies the delta: rebuilds the attribute state it captured.
    /// The label/framework side is external plumbing, so this returns the
    /// restored named shape instead of mutating a TDF label.
    pub fn apply(&self) -> TNamingNamedShape {
        let mut ns = TNamingNamedShape::with_evolution(self.evolution);
        let n = self.old_shapes.len().max(self.new_shapes.len());
        for i in 0..n {
            let old = self.old_shapes.get(i).cloned().unwrap_or_default();
            let new = self.new_shapes.get(i).cloned().unwrap_or_default();
            ns.add_shape(old, new);
        }
        ns
    }
