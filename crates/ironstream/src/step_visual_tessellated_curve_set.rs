// FILE: step_visual_tessellated_curve_set.rs
// occt: StepVisual_TessellatedCurveSet

use std::sync::Arc;

pub struct HasciiString;
pub struct CoordinatesList;
pub struct TessellatedItem;

pub struct TessellatedCurveSet {
    name: Option<Arc<HasciiString>>,
    coordinates: Option<Arc<CoordinatesList>>,
}

impl TessellatedCurveSet {
    pub fn new() -> Self {
        TessellatedCurveSet {
            name: None,
            coordinates: None,
        }
    }

    pub fn name(&self) -> Option<&Arc<HasciiString>> {
        self.name.as_ref()
    }

    pub fn set_name(&mut self, name: Option<Arc<HasciiString>>) {
        self.name = name;
    }

    pub fn coordinates(&self) -> Option<&Arc<CoordinatesList>> {
        self.coordinates.as_ref()
    }

    pub fn set_coordinates(&mut self, coords: Option<Arc<CoordinatesList>>) {
        self.coordinates = coords;
    }
}

impl Default for TessellatedCurveSet {
    fn default() -> Self {
        Self::new()
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_new() {
        let tcs = TessellatedCurveSet::new();
        assert!(tcs.name().is_none());
        assert!(tcs.coordinates().is_none());
    }

    #[test]
    fn test_set_and_get_name() {
        let mut tcs = TessellatedCurveSet::new();
        let name = Arc::new(HasciiString);
        tcs.set_name(Some(name));
        assert!(tcs.name().is_some());
    }

    #[test]
    fn test_set_and_get_coordinates() {
        let mut tcs = TessellatedCurveSet::new();
        let coords = Arc::new(CoordinatesList);
        tcs.set_coordinates(Some(coords));
        assert!(tcs.coordinates().is_some());
    }
}
