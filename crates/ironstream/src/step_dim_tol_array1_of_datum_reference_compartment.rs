// FILE: step_dim_tol_array1_of_datum_reference_compartment.rs
// occt: StepDimTol_Array1OfDatumReferenceCompartment

/// Rust port of OCCT's deprecated StepDimTol_Array1OfDatumReferenceCompartment.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepDimTolArray1OfDatumReferenceCompartment {
    data: Vec<String>,
    lower: i32,
}

impl StepDimTolArray1OfDatumReferenceCompartment {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepDimTolArray1OfDatumReferenceCompartment::new(1, 4);
    ...
    fn test_array1_access() {
        let mut arr = StepDimTolArray1OfDatumReferenceCompartment::new(1, 2);
        arr.set_value(1, "comp1".to_string());
        arr.set_value(2, "comp2".to_string());

        assert_eq!(arr.value(1), Some(&"comp1".to_string()));
        assert_eq!(arr.value(2), Some(&"comp2".to_string()));
    ...
    fn test_array1_from_vec() {
        ...
        let arr = StepDimTolArray1OfDatumReferenceCompartment::from_vec(5, values);

        assert_eq!(arr.lower_bound(), 5);
        assert_eq!(arr.upper_bound(), 6);
        assert_eq!(arr.value(5), Some(&"a".to_string()));
        assert_eq!(arr.value(6), Some(&"b".to_string()));
    }
}
