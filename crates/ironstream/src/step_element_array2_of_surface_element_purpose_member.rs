// FILE: step_element_array2_of_surface_element_purpose_member.rs
// occt: StepElement_Array2OfSurfaceElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_Array2OfSurfaceElementPurposeMember.
/// A 2D 1-based array wrapping a Vec in row-major order, mimicking NCollection_Array2 semantics.
#[derive(Clone, Debug)]
pub struct StepElementArray2OfSurfaceElementPurposeMember {
    data: Vec<String>,
    row_lower: i32,
    row_upper: i32,
    col_lower: i32,
...
impl StepElementArray2OfSurfaceElementPurposeMember {
...
mod tests {
    ...
    fn test_array2_creation() {
        let arr = StepElementArray2OfSurfaceElementPurposeMember::new(1, 3, 1, 2);
    ...
    fn test_array2_access() {
        let mut arr = StepElementArray2OfSurfaceElementPurposeMember::new(1, 2, 1, 2);
        arr.set_value(1, 1, "s11".to_string());
        arr.set_value(2, 2, "s22".to_string());

        assert_eq!(arr.value(1, 1), Some(&"s11".to_string()));
        assert_eq!(arr.value(2, 2), Some(&"s22".to_string()));
    }

    #[test]
    fn test_array2_bounds() {
        let arr = StepElementArray2OfSurfaceElementPurposeMember::new(1, 2, 1, 2);
