// FILE: step_element_h_array1_of_curve_element_section_definition.rs
// occt: StepElement_HArray1OfCurveElementSectionDefinition

/// Rust port of OCCT's deprecated StepElement_HArray1OfCurveElementSectionDefinition.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepElementHArray1OfCurveElementSectionDefinition {
    data: Vec<String>,
    lower: i32,
}

impl StepElementHArray1OfCurveElementSectionDefinition {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepElementHArray1OfCurveElementSectionDefinition::new(1, 4);
    ...
    fn test_harray1_access() {
        let mut arr = StepElementHArray1OfCurveElementSectionDefinition::new(1, 2);
        arr.set_value(1, "sect_def1".to_string());
        arr.set_value(2, "sect_def2".to_string());

        assert_eq!(arr.value(1), Some(&"sect_def1".to_string()));
        assert_eq!(arr.value(2), Some(&"sect_def2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["sd1".to_string(), "sd2".to_string(), "sd3".to_string()];
        let arr = StepElementHArray1OfCurveElementSectionDefinition::from_vec(1, values);

        assert_eq!(arr.length(), 3);
        assert_eq!(arr.value(2), Some(&"sd2".to_string()));
    }
}
