// FILE: step_geom_array2_of_surface_patch.rs
// occt: StepGeom_Array2OfSurfacePatch

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Array2<StepGeom_SurfacePatch>.
/// Provides 2D array with 1-based indexing semantics.
pub struct StepGeomArray2OfSurfacePatch {
...
impl StepGeomArray2OfSurfacePatch {
...
mod tests {
    ...
    fn test_new_array_2d() {
        let arr = StepGeomArray2OfSurfacePatch::new(1, 3, 1, 4);
    ...
    #[test]
    fn test_2d_indexing() {
        let mut arr = StepGeomArray2OfSurfacePatch::new(1, 2, 1, 3);

        arr.set_value(1, 1, Some("p11".to_string()));
        arr.set_value(1, 2, Some("p12".to_string()));
        arr.set_value(2, 1, Some("p21".to_string()));
        arr.set_value(2, 3, Some("p23".to_string()));

        assert_eq!(arr.value(1, 1), Some(&Some("p11".to_string())));
        assert_eq!(arr.value(1, 2), Some(&Some("p12".to_string())));
        assert_eq!(arr.value(2, 1), Some(&Some("p21".to_string())));
        assert_eq!(arr.value(2, 3), Some(&Some("p23".to_string())));
    }

    #[test]
    fn test_bounds_checking() {
        let arr = StepGeomArray2OfSurfacePatch::new(1, 2, 1, 3);
