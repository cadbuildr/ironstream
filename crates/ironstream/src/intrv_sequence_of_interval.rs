// FILE: intrv_sequence_of_interval.rs
// occt: Intrv_SequenceOfInterval

/// Deprecated alias for NCollection_Sequence<Intrv_Interval>.
/// Maintains backward compatibility. Use Vec directly in new code.
pub struct IntrvSequenceOfInterval {
    items: Vec<u32>, // Placeholder for Intrv_Interval (opaque type)
}

impl IntrvSequenceOfInterval {
    pub fn new() -> Self {
        Self {
            items: Vec::new(),
        }
...
impl Default for IntrvSequenceOfInterval {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_sequence_append() {
        let mut seq = IntrvSequenceOfInterval::new();
        assert!(seq.is_empty());
    ...
    #[test]
    fn test_default() {
        let seq = IntrvSequenceOfInterval::default();
        assert!(seq.is_empty());
    }
}
