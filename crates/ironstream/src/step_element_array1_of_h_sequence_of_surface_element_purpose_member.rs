// FILE: step_element_array1_of_h_sequence_of_surface_element_purpose_member.rs
// occt: StepElement_Array1OfHSequenceOfSurfaceElementPurposeMember

/// Rust port of OCCT's deprecated StepElement_Array1OfHSequenceOfSurfaceElementPurposeMember.
/// A 1-based fixed-length array wrapping a Vec, mimicking NCollection_Array1 semantics.
#[derive(Clone, Debug)]
pub struct StepElementArray1OfHSequenceOfSurfaceElementPurposeMember {
    data: Vec<String>,
    lower: i32,
}

impl StepElementArray1OfHSequenceOfSurfaceElementPurposeMember {
...
mod tests {
    ...
    fn test_array1_creation() {
        let arr = StepElementArray1OfHSequenceOfSurfaceElementPurposeMember::new(1, 4);
    ...
    fn test_array1_access() {
        let mut arr = StepElementArray1OfHSequenceOfSurfaceElementPurposeMember::new(1, 2);
        arr.set_value(1, "surf_member1".to_string());
        arr.set_value(2, "surf_member2".to_string());

        assert_eq!(arr.value(1), Some(&"surf_member1".to_string()));
        assert_eq!(arr.value(2), Some(&"surf_member2".to_string()));
    }

    #[test]
    fn test_array1_from_vec() {
        let values = vec!["sm1".to_string(), "sm2".to_string()];
        let arr = StepElementArray1OfHSequenceOfSurfaceElementPurposeMember::from_vec(1, values);

        assert_eq!(arr.length(), 2);
        assert_eq!(arr.value(1), Some(&"sm1".to_string()));
    }
}
