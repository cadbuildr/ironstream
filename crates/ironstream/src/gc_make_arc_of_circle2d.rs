// FILE: gc_make_arc_of_circle2d.rs
// occt: GC_MakeArcOfCircle2d

/// Constructs arcs of circles in 2D plane.
pub struct MakeArcOfCircle2d {
    done: bool,
    alpha1: f64,
    alpha2: f64,
    sense: bool,
...
impl MakeArcOfCircle2d {
    /// Construct arc from angular bounds on circle.
    ...
    /// Construct arc from point and angle on circle.
    ...
    /// Construct arc between two points on circle.
...
mod tests {
    ...
    #[test]
    fn test_from_angles() {
        let arc = MakeArcOfCircle2d::from_angles(0.0, 1.57079632679, true);
        assert!(arc.is_done());
    ...
    fn test_from_point_angle() {
        let arc = MakeArcOfCircle2d::from_point_angle(3.14159265359, false);
        assert!(arc.is_done());
    ...
    fn test_from_two_points() {
        let arc = MakeArcOfCircle2d::from_two_points();
        assert!(arc.is_done());
    }

    #[test]
    fn test_from_three_points() {
        let arc = MakeArcOfCircle2d::from_three_points();
        assert!(arc.is_done());
    }
}
