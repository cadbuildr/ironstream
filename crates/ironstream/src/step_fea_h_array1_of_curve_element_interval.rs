// FILE: step_fea_h_array1_of_curve_element_interval.rs
// occt: StepFEA_HArray1OfCurveElementInterval

/// Rust port of OCCT's deprecated StepFEA_HArray1OfCurveElementInterval.
/// HArray1 is a heap-allocated wrapper. A 1-based fixed-length array wrapping a Vec.
#[derive(Clone, Debug)]
pub struct StepFEAHArray1OfCurveElementInterval {
    data: Vec<String>,
    lower: i32,
}

impl StepFEAHArray1OfCurveElementInterval {
...
mod tests {
    ...
    fn test_harray1_creation() {
        let arr = StepFEAHArray1OfCurveElementInterval::new(1, 5);
    ...
    fn test_harray1_access() {
        let mut arr = StepFEAHArray1OfCurveElementInterval::new(1, 2);
        arr.set_value(1, "interval1".to_string());
        arr.set_value(2, "interval2".to_string());

        assert_eq!(arr.value(1), Some(&"interval1".to_string()));
        assert_eq!(arr.value(2), Some(&"interval2".to_string()));
    }

    #[test]
    fn test_harray1_from_vec() {
        let values = vec!["i1".to_string(), "i2".to_string()];
        let arr = StepFEAHArray1OfCurveElementInterval::from_vec(1, values);

        assert_eq!(arr.length(), 2);
        assert_eq!(arr.value(1), Some(&"i1".to_string()));
    }
}
