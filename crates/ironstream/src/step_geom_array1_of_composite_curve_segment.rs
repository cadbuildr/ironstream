// FILE: step_geom_array1_of_composite_curve_segment.rs
// occt: StepGeom_Array1OfCompositeCurveSegment

use std::vec::Vec;

/// Deprecated typedef alias for NCollection_Array1<StepGeom_CompositeCurveSegment>.
/// Provides 1-based indexing semantics with fixed bounds (Lower/Upper).
pub struct StepGeomArray1OfCompositeCurveSegment {
    data: Vec<Option<String>>,
    lower: usize,
}

impl StepGeomArray1OfCompositeCurveSegment {
...
mod tests {
    ...
    fn test_new_array_with_bounds() {
        let arr = StepGeomArray1OfCompositeCurveSegment::new(1, 5);
    ...
    fn test_one_based_indexing() {
        let mut arr = StepGeomArray1OfCompositeCurveSegment::new(1, 3);
        arr.set_value(1, Some("seg1".to_string()));
        arr.set_value(2, Some("seg2".to_string()));
        arr.set_value(3, Some("seg3".to_string()));

        assert_eq!(arr.value(1), Some(&Some("seg1".to_string())));
        assert_eq!(arr.value(2), Some(&Some("seg2".to_string())));
        assert_eq!(arr.value(3), Some(&Some("seg3".to_string())));
    }

    #[test]
    fn test_bounds_checking() {
        let arr = StepGeomArray1OfCompositeCurveSegment::new(1, 3);
